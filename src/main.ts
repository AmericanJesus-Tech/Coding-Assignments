//Cars
const modelContainer = document.getElementById("model-container") as HTMLDivElement

 export async function onFetchModelClick() {
    const response = await fetch("http://localhost:5173/")
    const ModelList = await response.json()

    modelContainer.innerHTML = ModelList.map(
        (        Model: { Model: any; ModelId: any; }) => `<div>
            <h3>${Model.Model}</h3>
            <p>${Model.ModelId}</P>
        `
    ).join("")
...
//class Car
    ...
    class Car{
        make: string | null;
        model: string | null;
        constructor(make: string | null, model: string | null){
            this.make = make;
            this.model = model;
        }
    }
    ...
    class Menu{
        ...
        //start the menu
        start(){
            let selection: any = this.showMainMenu();
